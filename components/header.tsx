  ...
  return (
    <div className="bg-sol-cream border-b border-sol-beige-deep sticky top-0 z-20">
      <div className="page-container py-3 flex items-center justify-between">
        <Link href="/" className="flex items-center" aria-label="Sol Noosa Car Rentals home">
          <Image src={LogoImage} alt="Sol Noosa Car Rentals" height={56} className="h-14 w-auto" />
        </Link>
          ...
          <nav className="md:flex items-center gap-1 hidden">
            {headerLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="rounded-full px-3.5 py-2 text-sm font-medium text-sol-brown-deep hover:bg-sol-brown/[0.06] transition-colors"
              >
                {link.label}
              </Link>
            ))}
